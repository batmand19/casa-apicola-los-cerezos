...
const PRODUCTS = [
  ...
  },
  {
    ...
    description: {
      ...
      sensory: { color: 'Tonos dorados a anaranjados, granulado fino', aroma: 'Fresco, floral, con notas terrosas y un toque dulce', sabor: 'Ligeramente dulce con matices florales y un final ácido', textura: 'Granulado suave, se disuelve fácilmente en líquidos' },
...
function ProductCard({ product }) {
  ...
  useEffect(() => {
    trackViewProduct({ id: product.id, name: product.name, category: product.category, size: selectedSize, price: selectedPrice });
  ...
  const handleSizeChange = useCallback((newSize) => {
    if (newSize !== selectedSize) {
      trackSelectVariant({ productId: product.id, productName: product.name, oldSize: selectedSize, newSize, price: product.sizes.find((s) => s.label === newSize)?.price || 0 });
      ...
      <div className="relative aspect-[4/3] bg-gradient-to-br from-cream-200 to-earth-100 overflow-hidden">
        ...
        <div className="absolute inset-0 img-placeholder">
          <div className="text-center p-6">
            ...
            <p className="text-sm text-earth-500">{product.name}</p>
            <code className="text-[10px] text-earth-400/60 block mt-1">{currentImage}</code>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-1.5 mb-5">
          {Object.entries({ Color: product.description.sensory.color, Sabor: product.description.sensory.sabor }).map(([label, value]) => (
            <p key={label} className="text-sm text-earth-500 flex items-start gap-2">
              <span className="font-semibold text-earth-700 flex-shrink-0">{label}:</span>
              <span>{value}</span>
            </p>
          ))}
        </div>
    ...
    <section id="productos" className="py-24 sm:py-32 bg-cream-50" aria-labelledby="products-title">
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        ...
        <div className="mt-12 text-center">
          <p className="text-sm text-earth-400">🚚 <strong>Envío gratis</strong> en compras superiores a ${FREE_SHIPPING_THRESHOLD.toLocaleString('es-CO')} COP</p>
        </div>
      </div>
    </section>
  );
