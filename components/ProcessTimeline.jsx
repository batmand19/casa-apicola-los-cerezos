...
const STEPS = [
  { icon: '🌸', title: 'Recolección', description: 'Las abejas Meliphera recolectan néctar de flores nativas: Eucalipto, Romero, Salvia, Uva de anís y Arrayán.' },
  { icon: '🧪', title: 'Transformación', description: 'En la colmena, el néctar se mezcla con enzimas especiales y se evapora el agua hasta concentrarse.' },
  ...
  { icon: '📦', title: 'Envasado artesanal', description: 'Filtramos suavemente y envasamos sin calor excesivo para preservar todas las propiedades.' },
];
    ...
    <section className="py-24 sm:py-32 bg-forest-900 text-cream-100 relative overflow-hidden" aria-labelledby="process-title">
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8 relative z-10">
        <div className="text-center mb-16 sm:mb-20">
          ...
          <h2 id="process-title" className="font-display text-3xl sm:text-4xl md:text-5xl font-bold text-cream-50">
            Cómo producimos nuestra miel
          </h2>
          <p className="mt-5 text-base text-cream-300/60 max-w-xl mx-auto">
            Un proceso artesanal que respeta a las abejas y preserva la pureza natural.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8 md:gap-6">
            {STEPS.map((step, i) => (
              <div key={i} className="relative flex md:flex-col items-start md:items-center gap-5 md:gap-0 text-left md:text-center group">
                {/* Nodo */}
                <div className="relative z-10 flex-shrink-0 w-16 h-16 rounded-2xl bg-gradient-to-br from-honey-500 to-honey-600 flex items-center justify-center text-2xl shadow-lg shadow-honey-500/20 group-hover:shadow-honey-500/40 transition-shadow duration-500">
                  {step.icon}
                </div>
                {/* Texto */}
